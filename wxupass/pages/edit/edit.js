...
Page({
    ...
    addsubitem:function(e){
        ...
        var strkey = '';
        var chcon = self.chcon();
        self.setData({ con:chcon });//只能用string 'con[0].con' 不能拼接字符串 , 只好全局更新
        //这里有一个问题单次设置setData数据量不能超过1024k
    },
    additem:function(){
        var self = this;
        var con = self.csd.con;
    ...
    delitem:function(e){ //console.log('delitem');
        var self = this;
        var ind = parseInt( e.currentTarget.dataset.ind );
        var con = self.csd.con;
        if(con.length <= 1){
    ...
    delsubitem:function(e){ //console.log('delsub');
        var self = this; 
        var ind = parseInt(e.currentTarget.dataset.ind );
    ...
    //同步保存输入的标题数据
    titlesave:function(e){ //console.log('blur'); console.log(e);
      var self = this;
      var ind = parseInt(e.currentTarget.dataset.ind);
      var con = self.csd.con;
    ...
    //同步保存键值内容
    kvsave:function(e){ //console.log('blur');
      var self = this;
      var ind = parseInt(e.currentTarget.dataset.ind);
      var sid = parseInt(e.currentTarget.dataset.sid);
